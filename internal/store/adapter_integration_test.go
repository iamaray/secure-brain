...
func TestPersistedRecordsAndRelationOrderingIntegration(t *testing.T) {
	...
	if got := []domain.BrainID{config.AllowedBrains[0].CanonicalID, config.AllowedBrains[1].CanonicalID}; !slices.Equal(got, []domain.BrainID{"brain.anish", "brain.atlas"}) {
		t.Fatalf("Brain grant order = %v", got)
	}
	if got := []domain.ServiceID{config.AllowedServices[0].CanonicalID, config.AllowedServices[1].CanonicalID}; !slices.Equal(got, []domain.ServiceID{"service.notion", "service.obsidian"}) {
	...
	if got := []domain.RecordID{config.Hops[0].ServiceID, config.Hops[1].ServiceID, config.Hops[2].ServiceID}; !slices.Equal(got, []domain.RecordID{seeds.obsidian.ID, seeds.notion.ID, seeds.obsidian.ID}) {
	...
	execution, err = db.TransitionExecution(ctx, execution.ID,
		domain.FailExecutionWithResult(errorCode, errorMessage, json.RawMessage(`{"attempts":1}`), startedAt, completedAt))
	if err != nil {
	...
	for _, viewer := range []string{mayaUserID, atlasUserID} {
		events, err := db.ListAuditEvents(ctx, domain.RecordID(viewer), AuditFilter{NodeID: domain.Principal(seeds.maya.CanonicalID)})
		if err != nil || len(events) != 1 || events[0].ID != domain.RecordID(auditID) {
			t.Fatalf("audit viewer %s = %#v, %v", viewer, events, err)
		}
	}
	if events, err := db.ListAuditEvents(ctx, domain.RecordID(anishUserID), AuditFilter{}); err != nil || len(events) != 0 {
	...
	record, err := db.CreateIdempotencyRecord(ctx, domain.RecordID(mayaUserID), "integration", "scanner-key", fmt.Sprintf("%064d", 9), time.Now().Add(time.Hour))
	if err != nil {
	...
	loadedRecord, err := db.GetIdempotencyRecord(ctx, domain.RecordID(mayaUserID), "integration", "scanner-key")
...
func TestTransferLifecycleAndIdempotencyCollisionsIntegration(t *testing.T) {
	...
	first, err := db.CreateIdempotencyRecord(ctx, domain.RecordID(mayaUserID), "transfer:accept", "collision-key", hashA, now.Add(time.Hour))
	if err != nil {
		t.Fatalf("create idempotency record: %v", err)
	}
	for name, hash := range map[string]string{"same request": hashA, "different request": hashB} {
		t.Run(name, func(t *testing.T) {
			if _, err := db.CreateIdempotencyRecord(ctx, domain.RecordID(mayaUserID), "transfer:accept", "collision-key", hash, now.Add(time.Hour)); !errors.Is(err, ErrIdempotencyExists) {
	...
	if _, err := db.CreateIdempotencyRecord(ctx, domain.RecordID(mayaUserID), "transfer:reject", "collision-key", hashB, now.Add(time.Hour)); err != nil {
...
func TestAuditAndTransferPaginationTiesIntegration(t *testing.T) {
	...
	events, err := db.ListAuditEvents(ctx, domain.RecordID(mayaUserID), AuditFilter{EventType: "pagination.tie", Limit: 2})
	if err != nil {
	...
	events, err = db.ListAuditEvents(ctx, domain.RecordID(mayaUserID), AuditFilter{EventType: "pagination.tie", Before: &tiedAt, Limit: 10})
...
func TestTransferRowLockSerializesContendingWritersIntegration(t *testing.T) {
	...
	source, err := db.CreateBrain(ctx, domain.RecordID(mayaUserID), fmt.Sprintf("contention-source-%d", unique), "Contention source")
	if err != nil {
		t.Fatalf("create contention source: %v", err)
	}
	destination, err := db.CreateBrain(ctx, domain.RecordID(atlasUserID), fmt.Sprintf("contention-destination-%d", unique), "Contention destination")
	if err != nil {
