...
func TestExecuteHopsRecordsRepeatedServicesInOrder(t *testing.T) {
	...
	out, hops, err := ExecuteHops(context.Background(), executor, services, in)
	if err != nil {
		t.Fatal(err)
	}
...
func TestExecuteHopsDetectsMutationAndStops(t *testing.T) {
	...
	out, hops, err := ExecuteHops(context.Background(), executor, services, domain.Payload{Bytes: []byte("abc")})
...
func TestExecuteHopsDetectsEnvelopeAndInPlaceMetadataMutation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, hops, err := ExecuteHops(context.Background(), executor, []domain.Service{{CanonicalID: "service.one"}}, input)
...
func TestExecuteHopsWrapsExecutorFailureAndStops(t *testing.T) {
	...
	out, hops, err := ExecuteHops(context.Background(), executor, []domain.Service{{CanonicalID: "service.one"}, {CanonicalID: "service.two"}}, domain.Payload{Bytes: []byte("abc")})
...
func TestExecuteHopsHonorsCancellationAndZeroHops(t *testing.T) {
	...
	out, hops, err := ExecuteHops(context.Background(), nil, nil, in)
	if err != nil || len(hops) != 0 || !reflect.DeepEqual(out, in) {
		t.Fatalf("zero-hop route failed: out=%#v hops=%#v err=%v", out, hops, err)
	}
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	_, hops, err = ExecuteHops(ctx, IdentityServiceExecutor{}, []domain.Service{{CanonicalID: "service.one"}}, in)
...
func TestExecuteHopsRejectsMoreThanSQLHopLimit(t *testing.T) {
	services := make([]domain.Service, DefaultMaxRouteHops+1)
	_, hops, err := ExecuteHops(context.Background(), IdentityServiceExecutor{}, services, domain.Payload{Bytes: []byte("x")})
