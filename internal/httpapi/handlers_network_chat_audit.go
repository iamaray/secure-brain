...
func (a *API) getNetwork(w http.ResponseWriter, r *http.Request) {
	...
	for _, route := range routeRows {
		...
		for index, hop := range route.Hops {
			edges = append(edges, map[string]any{"id": route.RouteID + ":" + itoa(index), "route_id": route.RouteID, "query_path_id": route.QueryPathID, "hop_index": index, "from": from, "to": hop.CanonicalID, "kind": "service_route", "label": route.SourceCanonicalID + ":" + route.Path})
		...
		edges = append(edges, map[string]any{"id": route.RouteID + ":" + itoa(hopIndex), "route_id": route.RouteID, "query_path_id": route.QueryPathID, "hop_index": hopIndex, "from": from, "to": terminalNode, "kind": "service_route", "label": route.SourceCanonicalID + ":" + route.Path})
	}
...
func (a *API) listAuditEvents(w http.ResponseWriter, r *http.Request) {
	...
	events, err := a.store.ListAuditEvents(r.Context(), activeUser(r.Context()).ID, store.AuditFilter{NodeID: r.URL.Query().Get("node_id"), EventType: r.URL.Query().Get("event_type"), Status: status, Limit: a.limits.DefaultPageSize})
	if err != nil {
		writeError(w, r, databaseError(err))
		return
	}
...
func (a *API) postChat(w http.ResponseWriter, r *http.Request) {
	...
	defer cancel()
	response, err := a.chat.Respond(ctx, openaiapi.Request{Model: a.chatModel, Instructions: openaiapi.BrainInstructions(brain.DisplayName, brain.CanonicalID), Input: input, MaxOutputTokens: a.limits.ChatMaxOutputTokens})
	if err != nil {
		a.audit(r, "chat.failed", "brain", brain.ID, &brain.ID, nil, nil, domain.AuditStatusFailed, map[string]any{"error_code": domain.CodeChatProviderError}, []string{activeUser(r.Context()).ID})
		writeError(w, r, err)
		return
	}
	...
	a.audit(r, "chat.requested", "brain", brain.ID, &brain.ID, nil, nil, domain.AuditStatusSucceeded, map[string]any{"model": a.chatModel}, []string{activeUser(r.Context()).ID})
	writeData(w, r, http.StatusOK, map[string]any{"messages": messages, "grounded": false, "disclosure": "This simulated Brain chat is not grounded in uploaded files."})
}
