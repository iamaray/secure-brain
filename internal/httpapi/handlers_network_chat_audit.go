...
func (a *API) listAuditEvents(w http.ResponseWriter, r *http.Request) {
	...
	events, err := a.store.ListAuditEvents(r.Context(), activeUser(r.Context()).ID, store.AuditFilter{NodeID: r.URL.Query().Get("node_id"), EventType: r.URL.Query().Get("event_type"), Status: status, Limit: 50})
	if err != nil {
		writeError(w, r, databaseError(err))
		return
	}
