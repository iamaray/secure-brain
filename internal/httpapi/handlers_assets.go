...
func (a *API) assetContent(w http.ResponseWriter, r *http.Request) {
	...
	preview, err := assets.BuildPreview(asset.Format, asset.ProcessingState, data, int(a.limits.MaxPreviewBytes), min(a.limits.MaxCSVRows, a.limits.MaxCSVPreviewRows))
	if err != nil {
